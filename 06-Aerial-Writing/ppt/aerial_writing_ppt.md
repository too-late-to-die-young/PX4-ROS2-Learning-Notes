...
自由飞行阶段，侵入深度 < 0，接触力为 0；接触书写阶段，笔尖进入虚拟侵入区域，接触力随深度线性增长。
...
---
# 03. 轨迹生成
**末端执行器轨迹 ${}_W\mathbf{r}_E^r, {}_W\mathbf{v}_E^r$：** 采用恒加速度运动模型，将需要书写的字符转换为末端执行器的平滑运动轨迹。

**机身轨迹 ${}_W\mathbf{r}_B^r, {}_W\mathbf{v}_B^r$：**
$$
\begin{aligned}
{}_W \mathbf{r}_B^r &= {}_W \mathbf{r}_E^r - \mathbf{C}_{WB} {}_B \mathbf{r}_{E_0} \\
{}_W \mathbf{v}_B^r &= {}_W \mathbf{v}_E^r - \mathbf{C}_{WB} \left( {}_B \boldsymbol{\omega}^r \times {}_B \mathbf{r}_{E_0} \right)
\end{aligned}
$$
其中 ${}_B \mathbf{r}_{E_0}$ 为末端执行器的标称位置，在该位置下，认为质心无偏移。 按照末端执行器始终在标称位置的情况设计机身轨迹。
...
## 多目标统一优化
$L$ 和 $\Phi$ 的代价形式为 $\boldsymbol{e}_i^\top \boldsymbol{Q}_i \boldsymbol{e}_i$ ，$\forall \boldsymbol{e}_i \in \{ \boldsymbol{e}_{rB}, \boldsymbol{e}_{rE}, \boldsymbol{e}_v, \boldsymbol{e}_\omega, \boldsymbol{e}_q, \boldsymbol{e}_f, \boldsymbol{e}_u \}$
...
$$\begin{aligned}
\mathbf{f}^* = \arg \min_{\mathbf{f}} & \left( \left\Vert \mathbf{A}\mathbf{f} - \mathbf{u}_{0, 1:4}^* \right\Vert_{\mathbf{W}}^2 + \lambda \Vert\mathbf{f}\Vert_2^2 \right) \\
...
## 末端执行器指令
通过求解 Delta 并联机械臂正逆运动学，由末端执行器位置计算得到三个电机的转动角度。
...
规划器通过最小化以下代价函数，生成动力学可行的状态与控制序列：

$$\min_{\mathbf{x}_{1:N}, \boldsymbol{\tau}_{a1:N}} \sum_{n=1}^{N} \left( \Vert\mathbf{v}_n\Vert_{W_v}^2 + \Vert\boldsymbol{\tau}_{an}\Vert_{W_{\tau}}^2 + \Vert\delta\boldsymbol{\tau}_{an}\Vert_{W_{d\tau}}^2 + \gamma \right) \cdot h_n$$

- $\Vert\mathbf{v}_n\Vert_{W_v}^2$：**速度惩罚**。本文使用全驱动无人机，无需最小化 $jerk$ 或 $snap$。
- $\Vert\boldsymbol{\tau}_{an}\Vert_{W_{\tau}}^2$：**能耗惩罚**。
- $\Vert\delta\boldsymbol{\tau}_{an}\Vert_{W_{d\tau}}^2$：**平顺性惩罚**。
...
| **研究侧重** | 欠驱动平台与机械臂重心协同，笔画跟踪 | 复杂接触力控制 |
